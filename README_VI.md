...
### Thêm upstream API tùy chỉnh vào pool

Trong trang admin, chọn **Thêm tài khoản → Upstream API tùy chỉnh**, sau đó chọn
**OpenAI — Chat Completions** hoặc **Anthropic — Messages / Claude Code**. URL lấy từ
`ANTHROPIC_BASE_URL` trong `settings.json` của Claude Code phải chọn Anthropic. Tiếp theo nhập
Base URL, API key, danh sách model (mỗi dòng một model, hỗ trợ wildcard như `gpt-*`), trọng số
và outbound proxy tùy chọn. Nguồn này dùng chung routing, retry/cooldown, streaming, tool calls
và thống kê token với pool hiện có. URL từ xa phải dùng HTTPS; HTTP chỉ được phép với loopback.

Mỗi custom upstream có thể cấu hình đơn giá input, output, cache read và cache write theo
...
| `ADMIN_PATH` | Prefix URL của panel & API admin (ví dụ `/panel-x7k9`). Hoạt động như một bí mật chia sẻ: khi đặt, đường dẫn `/admin` cũ trả 404 trơn như mọi route lạ. Không bao giờ ghi vào config. | `/admin` |
